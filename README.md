...
- **CardsModule**: Handles card addition, Luhn validation, and secure tokenization.
- **PaymentsModule**: Manages the transaction state machine and idempotency.
- **MockBankModule**: Internal mock API for simulating bank responses and network latencies.
...
### 6. Running Tests
The project comes with a test suite configured using Jest. To run the automated tests and verify the code:
```bash
npm run test
```
